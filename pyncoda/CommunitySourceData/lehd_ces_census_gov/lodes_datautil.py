...
from pyincoredata_addons.SourceData.lehd_ces_census_gov.\
    _lodes_data_structure import all_segstems
...
def download_lodes(year: str, 
    ...
    soucerurl = 'https://lehd.ces.census.gov/data/lodes/LODES7'
...
def check_out_of_state_rac(df):
    ...
    for prefix in ['h','w']:
        df.loc[:, prefix+'_geocode'+'_str'] = df[prefix+'_geocode'].apply(lambda x : str(int(x)).zfill(15))
        df.loc[:, prefix+'_stfips'] = df[prefix+'_geocode'+'_str'].str[0:2]
        # Create lower case state abbrevation based on fips code
        df.loc[:, prefix+'_stabbr'] = df[prefix+'_stfips'].apply(lambda x :  str.lower(us.states.lookup(x).abbr))
        df.loc[:, prefix+'_geocode'+'_str'] = df[prefix+'_geocode'].apply(lambda x : str(int(x)).zfill(15))
...
def import_lodes(year: str, 
    ...
    xwalk_df = pd.read_csv(filepaths[1], usecols = ['tabblk2010','cty'])
    ...
    for geocode_var in geocode_vars:
        ...
        df = df.drop(columns = ['tabblk2010','cty'])
...
def expand_df(df, expected_count):
    ...
    if negative_jobcount_len > 0:
        ...
        df.loc[df[expected_count] < 0, expected_count] = 1
...
def add_latlon(df):
    ...
    for work_state in work_state_list:
        ...
        # Read in Crosswalk between blocks and County FIPS code
        xwalk_df_work = pd.read_csv(xwalk_filepath_work, usecols = ['tabblk2010','blklatdd','blklondd'])
        ...
        for home_state in home_state_list:
            ...
            for geocode_var in geocode_vars:
                ...
                # Read in Crosswalk between blocks and County FIPS code
                xwalk_df_home = pd.read_csv(xwalk_filepath_home, usecols = ['tabblk2010','blklatdd','blklondd'])
                ...
                # Rename lat lon vars
                rename_dict = {}
                for var in ['tabblk2010','blklatdd','blklondd']:
