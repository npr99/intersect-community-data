...
from . import get_homeblocklist
from . import import_lodes
from . import new_jobtypes
from . import fix_char_vars
from . import stack_jobset
from . import keep_nonzeros
from . import explorebyblock
from . import check_out_of_state_rac
from . import reshapecascade
from . import expand_df
from . import add_jobids
from . import wac_rac_joblist
from . import wac_rac_od_joblist
from . import remove_duplicate_block_error
from . import add_total_count_byvar
from . import add_jobidod_pair

# Use LODES Data Structure set of dictionaires to setup loops
from  . import all_ods, all_segparts, all_jobtypes, all_segstems, all_mxjobtypes
...
def block_to_joblist(stacked_df,
    ...
    for year in years:
        ...
        for od in ['wac','rac']:
            for seg in ['SE','SI','SA']:
                reshaped = reshapecascade(dfselect[od,seg],reshape_vars)
                expanded = expand_df(reshaped,'jobcount')
                reshapecascade_df[od,seg] = add_jobids(expanded)

        # reshape od data frame
        reshaped = reshapecascade(dfselect['od','na'],odreshape_vars)
        expanded = expand_df(reshaped,'jobcount')
