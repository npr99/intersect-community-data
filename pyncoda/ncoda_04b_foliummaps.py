...
def plot_dotmap_map(gdf, 
    ...
    if focal_gdf is not None:
        marker_size = calculate_marker_size(focal_gdf)  
    else:
        marker_size = calculate_marker_size(gdf)
...
def plot_data_with_contextily(gdf, 
    ...
    if map_var == 'Household Income Group':
        # Define a custom colormap from red to green
        cmap = mcolors.LinearSegmentedColormap.from_list("income_cmap", ["red", "green"])
